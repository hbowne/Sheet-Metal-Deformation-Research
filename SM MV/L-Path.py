...
q_start=read_position(egm) #calc initial position
...
#traj_q = np.vstack((traj_q,traj_q_L))
#plt.plot(np.degrees(traj_q))
#plt.show()
...
traj_q_L, time_bp=trajectory_generate(curve_js,robot,lin_vel=tool_vel,lin_acc=tool_acc) # generate a smooth trajectory
curve_js_exe = []
plt.plot(traj_q_L)
plt.show()
for i in range(len(traj_q_L)):
    read_q = read_position(egm) # reading joint position take approximately 4 ms
